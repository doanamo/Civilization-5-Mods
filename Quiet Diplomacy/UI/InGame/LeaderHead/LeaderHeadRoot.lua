...
print("Quiet Diplomacy has been loaded!")
local qdLeaderView = false
...
function LeaderMessageHandler( iPlayer, iDiploUIState, szLeaderMessage, iAnimationAction, iData1 )
	...
	-- Quiet Diplomacy.
	local qdPlayer = Players[Game.GetActivePlayer()];
	local qdAIPlayer = Players[iPlayer];
	local qdAICapitalX = qdAIPlayer:GetCapitalCity():GetX()
    local qdAICapitalY = qdAIPlayer:GetCapitalCity():GetY()
    local qdNotificationIcon = NotificationTypes.NOTIFICATION_MINOR
    local qdNotificationHeader = "Message from " .. qdAIPlayer:GetName()
    local qdNotificationMessage = qdAIPlayer:GetName() .. ": " .. szLeaderMessage

	local function qdLeaverScreen()
		UI.SetLeaderHeadRootUp( false );
		UIManager:DequeuePopup( ContextPtr );
		UI.RequestLeaveLeader();
	end
    
    if not qdLeaderView then
        if (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_BLANK_DISCUSSION) then
            qdPlayer:AddNotification(qdNotificationIcon, qdNotificationMessage, qdNotificationHeader, qdAICapitalX, qdAICapitalY)
			qdLeaverScreen()
            return false
